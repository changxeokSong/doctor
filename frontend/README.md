...
### 표시되는 "점수"는 순수 임베딩 유사도가 아니다

백엔드가 답변 키워드와 사전 표제어를 임베딩 유사도로 비교한 뒤, **세부분류의 우선 카테고리에
속하는 표제어에는 가산점을 더한다**(`CATEGORY_BOOST`, 정확일치는 제외). 가산의 목적은 순서 조정이
아니라 **컷오프 구제** — 유사도가 근거 문턱을 아깝게 못 넘어 통째로 탈락하는 표제어를 살리기 위함이다.

따라서 화면의 "점수"와 "점수 순위"는 이 **보정값 기준**이다. 분석 페이지 3번 "점수 보정" 행에 명시돼 있다.
프론트는 원점수를 받지 못하므로 행별 가산 여부는 표시하지 않는다(백엔드가 원점수를 내려주면 가능).
