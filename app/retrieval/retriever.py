"""임베딩 + DB 검색(생성이 없는 순수 검색이라 'RAG'가 아니다 - 2026-07-26에 파일명도 rag.py에서
retriever.py로 바꿨다, 매뉴얼 "왜 RAG라고 안 부르나" 참고):
의사 질문을 임베딩해 코퍼스 질문들과 유사도 비교, 가장 비슷한 질문에 달린 실제 환자 답변을 가져온다.

근접중복 질문(표현은 다르지만 실질적으로 같은 질문, 예: LLM 증강으로 여러 말투가 생긴 경우)은 행을
지우지 않는다 — 각 표현을 검색 후보로 그대로 남겨야 그 특정 말투로 검색했을 때도 정확히 매칭되기
때문. 대신 '그룹대표ID' 컬럼(scripts/augmentation/group_near_duplicate_questions_20260724.py)으로
묶어두고, 답변 예시를 보여줄 때만 그룹 전체의 답변 풀을 합쳐서 준다."""
import os
...
def retrieve_answer(
    ...
    분류기 예측 1위 세부분류에만 검색을 가두면, 1위와 2위 확률 차이가 근소해서 실제로는 2위가
    맞는 경우(실측 확인: "MRI에서 디스크나 협착증 이야기를 들으셨나요?"가 코퍼스에 exam_history로
    토씨 하나 안 틀리고 존재하는데도, 분류기가 history(44.4%) > exam_history(17.5%)로 근소하게
    잘못 예측해서 검색 후보에 아예 못 들어갔던 사례가 있었다) 정답을 완전히 놓칠 수 있다.
    그래서 상위 후보 여러 개(예: top-3)를 한꺼번에 검색 대상에 포함할 수 있게 한다."""
